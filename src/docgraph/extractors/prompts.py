...
def build_system_prompt(
    ...
    lines = [
        ...
        *[f"- {t}" for t in relation_types],
        ...
        "6. 每条关系必须能读作「主语 + 关系类型 + 宾语」的自然句，且是对原文事实的准确转述（例如「劳动法 规定 女职工的特别保护」成立，而「女职工 属于 劳动法」不成立）；",
        "7. 若原文语义与类型表不匹配，宁可不抽取该关系，也不得硬套不匹配的关系类型；",
        "8. 禁止抽取无实质信息的关系，例如「某概念/主体 属于 某文件/某法律」这类泛化表述；「依据/基于」仅在原文明确表述来源或依据关系时使用。",
